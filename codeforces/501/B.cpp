...
void solve(){
    ll n;
    cin>>n;
    map<string,string> befor;
    map<string,ll> used;
    for(ll i=0;i<n;i++){
        string s1,s2;
        cin>>s1>>s2;
        if(!used[s2]){
            if(befor.find(s1)==befor.end()) befor[s1]=s1;
            if(befor.find(s2)==befor.end()) befor[s2]=befor[s1];
            used[s2]=1;
            used[s1]=0;
        }
    }
    ll ans=0;
    for(auto i:used) if(i.S==1) ans++;
    cout<<ans<<endl;
    for(auto i:used){
        if(i.S==1){
            cout<<befor[i.F]<<' '<<i.F<<endl;
        }
    }
}
